...
def _latest_two(conn: sqlite3.Connection, symbol: str, asof: date) -> list:
    # released_at <= asof ensures EIA / JODI / COT rows whose data hasn't
    # been published yet stay hidden when this report is built for a
    # historical date. Price-style symbols (lag=0) behave identically.
    rows = conn.execute(
...
def _latest_two_settled(conn: sqlite3.Connection, symbol: str, asof: date) -> list:
    ...
    """
    today = date.today()
    # released_at <= asof is the as-of cut (zero-lag for prices, so same
    # behaviour as price_time for WTI/Brent — but uniform with the rest
    # of the codebase). price_time < today still excludes today's
    # in-progress bar (a wall-clock gate, not a release gate).
    rows = conn.execute(
        "SELECT price_time, close FROM market_prices WHERE symbol=? "
        "AND released_at<=? AND price_time<? "
        "AND close IS NOT NULL ORDER BY price_time DESC LIMIT 2",
