...
- 二、市场因素: 编号列表, 用 "1)", "2)", "3)" 等半角数字加右括号, 不要用 markdown bullet 符号 (-, *)。每项 1-3 句话, 独立成段。覆盖 6-10 个项目, 按重要性排序:
  · **如果** 用户消息的"今日 EIA 库存数据"小节有数据, 第 1 项必须是该库存详情, 把所有提供的数字 (商业原油、库欣、汽油、馏分油及变化幅度) 用一段话写完, 风格同卖方周报示例。
  · **如果** "今日 EIA 库存数据"小节标注 "(本期无新数据)" 或不存在, **完全不要提及库存** — 不要复述上周或上次的库存数字, 不要写"库存数据待发布"之类的填充语。直接按地缘政治、政策、市场新闻等其他类别排序。
...
def _gather(asof: date) -> dict:
    ...
    for sym, label in inv_series:
        ...
        try:
            ...
            continue
        # Only include fresh inventory data — otherwise the LLM will
        # dutifully write up last week's print as if it were today's news.
...
def _format_user_prompt(ctx: dict) -> str:
    ...
    if not ctx["inventory"]:
        L.append("(本期无新数据 — 请勿在报告中提及库存, 不要复述以前的数字)")
