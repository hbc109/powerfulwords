...
- 二、市场因素: 编号列表, 用 "1)", "2)", "3)" 等半角数字加右括号, 不要用 markdown bullet 符号 (-, *)。每项 1-3 句话, 独立成段。覆盖 6-10 个项目, 按重要性排序:
  · **EIA 库存项目规则** (用户消息会预先给出"状态"标签作为判断依据, 你不需要自己做日期判断):
    ...
    – **唯一例外**: 即使状态是 "上一期已过去", 如果新闻正文 (下方文档摘录) 出现明确标注为本周 (week ending YYYY-MM-DD) 的 WPSR 引用, 且 week ending 日期与上方结构化库存的"week ending"日期相匹配, 那么可以在某一项中简短转述这条引用 (但不要再当作"今日新发布的数据")。如果不能 100% 确认日期匹配, 仍然完全跳过。
