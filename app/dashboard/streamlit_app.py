...
with tab_daily:
    ...
    st.caption(
        "Today's headlines + latest inventory. Generated by "
        "`scripts/daily_news_report.py` (no scores, no signal direction — "
        "just the facts). Cron'd nightly; you can also rebuild manually with "
        "the button below or pick a past date to view."
    )

    digest_dir = BASE_DIR / "data" / "processed" / "digests"
    available = sorted(
        [p for p in digest_dir.glob("daily_news_*.md")],
        reverse=True,
    ) if digest_dir.exists() else []
    available_dates = [p.stem.replace("daily_news_", "") for p in available]

    dc1, dc2 = st.columns([2, 1])
    with dc1:
        pick_default = available_dates[0] if available_dates else date.today().isoformat()
        picked_str = st.selectbox(
            "Report date",
            available_dates if available_dates else [date.today().isoformat()],
            index=0,
            key="daily_report_pick",
        )
    with dc2:
        if st.button("🔄 Rebuild for today", key="rebuild_daily_btn"):
            try:
                subprocess.run(
                    [sys.executable, str(BASE_DIR / "scripts" / "daily_news_report.py")],
                    check=True, cwd=str(BASE_DIR), capture_output=True,
                    timeout=30,  # hard cap — script normally finishes in 1-2s
                )
                st.success("Rebuilt — refresh to see the latest.")
                st.rerun()
            except subprocess.TimeoutExpired:
                st.error("Build timed out after 30s. Likely DB contention — try again in a moment.")
            except subprocess.CalledProcessError as e:
                st.error(f"Build failed: {e.stderr.decode()[-300:]}")

    md_path = digest_dir / f"daily_news_{picked_str}.md"
    if md_path.exists():
        st.markdown(md_path.read_text(encoding="utf-8"))
    else:
        st.info(f"No report for {picked_str} yet. Click rebuild above (only works for today).")
